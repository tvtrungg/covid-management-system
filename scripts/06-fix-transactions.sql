...
-- Tạo tài khoản EXTERNAL cho giao dịch nạp tiền từ bên ngoài
INSERT INTO payment_accounts (account_id, balance, is_main_account) VALUES 
('EXTERNAL', 0, false)
...
-- Thêm một số giao dịch mẫu khác
INSERT INTO transactions (from_account_id, to_account_id, amount, transaction_type, description, status) VALUES 
('EXTERNAL', 'USER_001', 200000, 'deposit', 'Nạp tiền bổ sung', 'completed'),
('EXTERNAL', 'USER_003', 150000, 'deposit', 'Nạp tiền bổ sung', 'completed'),
('USER_003', 'MAIN_ACCOUNT', 95000, 'payment', 'Thanh toán đơn hàng #4', 'completed');

-- Cập nhật lại số dư sau các giao dịch mới
UPDATE payment_accounts SET balance = 420000 WHERE account_id = 'USER_001'; -- +200000
UPDATE payment_accounts SET balance = 355000 WHERE account_id = 'USER_003'; -- +150000 -95000
UPDATE payment_accounts SET balance = 495000 WHERE account_id = 'MAIN_ACCOUNT'; -- +95000

-- Thêm một số người dùng và dữ liệu mẫu khác
INSERT INTO users (username, password_hash, role, first_login) VALUES 
('user003', '123123', 'user', false),
('user004', '123123', 'user', false),
('manager002', '123123', 'manager', false);

-- Thêm người liên quan Covid-19 khác
INSERT INTO covid_people (full_name, id_number, birth_year, province_id, district_id, ward_id, status, treatment_location_id, user_id) VALUES 
('Phạm Thị Dung', '789123456', 1992, 3, 11, NULL, 'F3', NULL, (SELECT id FROM users WHERE username = 'user003')),
('Hoàng Văn Em', '321654987', 1987, 1, 3, NULL, 'F2', 3, (SELECT id FROM users WHERE username = 'user004'));

-- Tạo tài khoản thanh toán cho người mới
INSERT INTO payment_accounts (account_id, balance, is_main_account, person_id) VALUES 
('USER_004', 100000, false, 4),
('USER_005', 250000, false, 5);

-- Thêm giao dịch cho người mới
INSERT INTO transactions (from_account_id, to_account_id, amount, transaction_type, description, status) VALUES 
('EXTERNAL', 'USER_004', 100000, 'deposit', 'Nạp tiền lần đầu', 'completed'),
('EXTERNAL', 'USER_005', 250000, 'deposit', 'Nạp tiền lần đầu', 'completed');
